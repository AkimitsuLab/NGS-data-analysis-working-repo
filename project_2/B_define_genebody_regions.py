...
for line in input_file:
    ...
    if strand == '+':
        ...
        print(chrom, str(tss_site_ed), str(ed_list[-1]), data[0], ','.join(trx_list_new), "GeneBody", sep="\t", end="\n", file=output_file)
        print(chrom, str(tss_site_st), str(tss_site_ed), data[0], ','.join(trx_list_new), "Promoter", sep="\t", end="\n", file=output_file_promoter)
    elif strand == '-':
        tss_list.sort()
        tss_site_st = tss_list[0]
        tss_site_ed = int(tss_site_st) + 3000
        print(chrom, str(st_list[0]), str(tss_site_st), data[0], ','.join(trx_list_new), "GeneBody", sep="\t", end="\n", file=output_file)
        print(chrom, str(tss_site_st), str(tss_site_ed), data[0], ','.join(trx_list_new), "Promoter", sep="\t", end="\n", file=output_file_promoter)
